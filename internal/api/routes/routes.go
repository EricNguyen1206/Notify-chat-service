...
func NewApp() (*App, error) {
	...
	DB, DbErr := database.NewPostgresConnection(config.Database.User, config.Database.Password, config.Database.Host, config.Database.Port, config.Database.DBName)

	if DbErr != nil {
		slog.Error("Init DB Failed:", DbErr.Error())
	}

	RedisInstance, RedisErr := database.NewRedisConnection(&config.Redis)

	if RedisErr != nil {
		slog.Error("Init Redis Failed:", RedisErr.Error())
	}

	redisClient := RedisInstance.GetClient()
	redisService := services.NewRedisService(RedisInstance)

	wsHub := websocket.NewHub(redisService)
	wsHub.Run()
	...
	// Service
	userService := services.NewUserService(userRepo, config.JWT.Secret, redisClient)
